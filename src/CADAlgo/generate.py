...
from CADAlgo.check import iter_check_line, is_angle_forbidden
from CADAlgo.parser import PartPointCandidate, PartLineCandidate, MyLine
import logging
...
def sample_points_from_polygon(polygon: Polygon, step: float) -> list[Point]:
    """
...
# TODO 线条需要不垂直于外围边界线, 应该是一个范围的角度内(例如15°至75°).
# TODO 优化建议, samples_distance 从大往小变化 可以根据外围盒子的周长的100份, 或者外围盒子的周长的目标个数份
# TODO 运行时间超时随机重试机制
def underlying_generate_solution(
    ...
    sampled_boundary_pts = sample_points_from_polygon(exterior, samples_distance)
    ...
    # 为每个候选预生成所有可能线(按优先级、长度升序)
    all_candidate_lines: list[list[MyLine]] = []
    counter = 0
    for candidate in total_candidates:
        lines_heap: list[tuple[float, float, int, MyLine]] = []
        for pt in candidate.points:
            pt_priority = pt.priority
            for boundary_pt in sampled_boundary_pts:
                # 直接构造 MyLine，优先级与 part_id 继承点
                start = (pt.geometry.x, pt.geometry.y)
                end = (boundary_pt.x, boundary_pt.y)
                line = MyLine(start, end, priority=pt_priority, part_id=pt.part_id)
                # 条件过滤: 角度 + 不穿外轮廓 + 不穿障碍 + 终点不靠驱散线
                if is_angle_forbidden(line.geometry):
                    continue
                if not line.geometry.crosses(exterior) and not any(
                    line.geometry.crosses(ob) for ob in obstacles
                ):
                    # 终点远离驱散线（提前到生成阶段过滤）
                    end_point = Point(end)
                    if any(end_point.distance(dl) <= 1 for dl in dispel_lines):
                        continue
                    # 优先按点的优先级排序, 其次按线长排序
                    heappush(
                        lines_heap, (pt_priority, line.geometry.length, counter, line)
                    )
                    counter += 1
        # 排序成列表(按优先级, 再按长度)
        sorted_lines: list[MyLine] = [item[3] for item in sorted(lines_heap)]

        all_candidate_lines.append(sorted_lines)

    best_solution: list[MyLine] = []
    logged_full = False  # avoid duplicate logs for multiple complete solutions

    def dfs(
        idx: int,
        selected_lines: list[MyLine],
    ) -> None:
        ...
        for line in all_candidate_lines[idx]:
            try_start = line.start
            try_end = line.end
            logger.debug(
                f"Try line at idx={idx}: start={try_start} end={try_end} len={float(line.geometry.length)}",
            )
            if iter_check_line(
                line.geometry,
                [s.geometry for s in selected_lines],
            ):
                logger.debug(
                ...
                logger.debug(
                    f"Backtrack at idx={idx}: pop line start={popped.start} end={popped.end}",
                )
